...
def train_epoch(model, training_data, optimizer, loss_fn, device, opt):
    ...
    for batch in tqdm(
        ...
        optimizer.zero_grad()
        # prepare data
        img_fea, tags = map(lambda x:x.to(device), batch[1])
        logits = model(img_fea)
...
def test(model, test_data, loss_fn, device, opt):
    ...
    with torch.no_grad():
        for batch in tqdm(
            ...
            img_fea, tags = map(lambda x:x.to(device), batch[1])
            logits = model(img_fea)
            pred_tag = F.sigmoid(logits)

            img_ids.extend(batch[0])
            #img_embedds.append(img_embedd)
            target_embedds.append(pred_tag)
            query_embedds.append(tags)
...
def train(model, training_data, validation_data, optimizer, loss_fn, device, opt):
    ...
    valid_losses = []
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.75, patience=0, verbose=True)
...
def main():
    ...
    loss_fn = nn.BCEWithLogitsLoss(F.softmax(torch.FloatTensor(vocab_tag.word_count).to(device), dim=-1)*10)
    ...
    #predict_prob(dan, validation_data, loss_fn, device, opt)
