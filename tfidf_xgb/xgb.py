...
Y_train = pca.transform(train['captions'])
Y_test = pca.transform(test['captions'])
...
if not os.path.exists(save_dir):
        os.makedirs(save_dir)
model.save_model(save_dir+'xgboost_trees_520_depth_4')
t = time.time()
...
predict_captions = model.predict(X_train)
...
predict_captions = model.predict(X_test)
